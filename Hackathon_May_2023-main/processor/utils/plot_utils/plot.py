...
def infotext(evID='', timestamp='', slopemax='', ntraces='', ntest='', baseline_subtracted=None, chi2=''):
    
    text = []
    
    if evID:
        text.append(r'Trace Index ' + str(evID))    
    if timestamp:
        text.append(rf'$t_0 = $ {timestamp:.2f} ms ?')
    if ntraces:
        text.append(r'$N_{traces}=$' + str(ntraces))
    if baseline_subtracted:
        text.append(r'baseline subtracted')
    if slopemax:
...
class Plotter:
    ...
    def __init__(self, config={}):
        ...
        self.title = config.get('title', "waveform plotter")
