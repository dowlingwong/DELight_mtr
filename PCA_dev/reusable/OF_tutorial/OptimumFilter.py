...
_CHISQ_MODE_ALL = "all"
_CHISQ_MODE_NONE = "none"
_VALID_CHISQ_MODES = (_CHISQ_MODE_ALL, _CHISQ_MODE_NONE)
...
@njit(cache=True)
def _slide_and_eval_generic_amponly(x, fs, F, E, X, start, hop, steps, N, amps, out_offset):
    t0 = start - hop
    for s in range(steps):
        for u in range(hop):
            t = t0 + u + s * hop
...
class OptimumFilter:
    """Frequency-domain optimum filter with optional sliding-window evaluation."""
    ...
    @staticmethod
    def _validate_chisq_mode(chisq_mode):
        if chisq_mode not in _VALID_CHISQ_MODES:
            raise ValueError("chisq_mode must be 'all' or 'none'.")

    @staticmethod
    def _unfold_noise_psd(noise_psd, n_samples):
        """Convert one-sided PSD to two-sided PSD for complex FFT convention."""
        if n_samples % 2 == 0:
    ...
    @staticmethod
    def _allocate_sliding_outputs(num_windows, chisq_mode):
        amps = np.empty(num_windows, dtype=np.float64)
        if chisq_mode == _CHISQ_MODE_NONE:
            chisqs = np.full(num_windows, np.nan, dtype=np.float64)
        else:
    ...
    def _update_state(self):
        ...
        self._noise_psd_unfolded = self._unfold_noise_psd(self._noise_psd, N)
    ...
    def fit(self, trace):
        ...
        chisq0 = (
            np.real((trace_fft.conj() * trace_fft / self._noise_psd_unfolded).sum())
    ...
    def _sliding_fit_hop1(self, x, num_windows, reanchor_every, chisq_mode, scale, kern_norm, amps, chisqs):
        """Fast blocked hop=1 path based on specialized Numba kernels."""
        N = self._length
        fs = self._sampling_frequency
            ...
            if chisq_mode == _CHISQ_MODE_ALL:
                ...
                if steps_in_block > 1:
                    _slide_block_hop1_amponly(
                        x, fs, self._F_real, self._F_imag, self._E_real, self._E_imag, X,
    ...
    def _sliding_fit_generic(self, x, num_windows, hop, reanchor_every, chisq_mode, kern_norm, amps, chisqs):
        """Generic hop>1 path using complex recurrence with optional re-anchoring."""
        N = self._length
        fs = self._sampling_frequency
        E = self._E
        ...
        if chisq_mode == _CHISQ_MODE_ALL:
            ...
            chisqs[0] = (chi00 - amp0 * amp0 * kern_norm) / (N - 2)
        ...
        while start <= L - N:
            if reanchor_every and (made % reanchor_every == 0):
                ...
                if chisq_mode == _CHISQ_MODE_ALL:
                    chi0 = _compute_chi0(X, S_unf, fs, N)
                    chisqs[made] = (chi0 - amp * amp * kern_norm) / (N - 2)
            ...
            if chisq_mode == _CHISQ_MODE_ALL:
                ...
                _slide_and_eval_generic_amponly(
                    x, fs, F, E, X,
                    start, hop, steps, N,
                    amps, made,
                )
    ...
    def sliding_fit(self, trace_long, hop=1, reanchor_every=None, chisq_mode='all'):
        ...
        self._validate_chisq_mode(chisq_mode)

        num_windows = 1 + (L - N) // hop
        amps, chisqs = self._allocate_sliding_outputs(num_windows, chisq_mode)
        scale = self._sampling_frequency / N
        kern_norm = float(self._kernel_normalization)

        if hop == 1:
            return self._sliding_fit_hop1(
                x, num_windows, reanchor_every, chisq_mode, scale, kern_norm, amps, chisqs
            )
        return self._sliding_fit_generic(
            x, num_windows, hop, reanchor_every, chisq_mode, kern_norm, amps, chisqs
        )
