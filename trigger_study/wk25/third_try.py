...
if __name__ == "__main__":
    ...
    for ENERGY_BIN in ENERGY_BINS:
        ...
        this_n_channels = int(attrs["n_channels"])
        this_trace_samples = int(attrs["trace_samples"])
        this_trace_dtype = np.dtype(attrs.get("trace_dtype", np.float16))

        if n_channels is None:
            n_channels = this_n_channels
            trace_samples = this_trace_samples
            trace_dtype = this_trace_dtype
        else:
            assert this_n_channels == n_channels, "n_channels mismatch across energy bins"
            assert this_trace_samples == trace_samples, "trace_samples mismatch across energy bins"
            assert this_trace_dtype == trace_dtype, "trace_dtype mismatch across energy bins"

        # ---- Build memmap once (idempotent) per energy bin
        ...
        ds_bin = MemmapEventDataset(mm_path, meta_df, n_channels, trace_samples, trace_dtype)
        datasets.append(ds_bin)

    # ---- Concatenate all energy bins into one dataset
    ds = ConcatDataset(datasets)
    print(f"Loaded {len(ds)} total events from energy bins {ENERGY_BINS}")
    print(f"Classes (from first dataset): {datasets[0].class_to_idx}  (num={datasets[0].num_classes})")
    ...
    checkpoint_path = BASE / "tcxformer_multienergy.pt"
    ...
    try:
        ...
        del model, optimizer, scaler, loader, ds, datasets
        gc.collect()
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
