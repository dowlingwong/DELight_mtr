...
class OptimumFilter():
    ...
    def _update_state(self):
        ...
        self._template_fft = sp_fft.fft(self._template) / self._sampling_frequency
    ...
    def fit(self, trace):  # fit only one trace
        trace_fft = sp_fft.fft(trace, axis=-1) / self._sampling_frequency  # V
    ...
    def fit_with_shift(self, trace, allowed_shift_range=None):
        ...
        trace_filtered_td = np.real(
            sp_fft.ifft(trace_filtered, axis=-1)
    ...
    def evaluate_trigger_rate(self, traces, threshold):
        """
        Evaluate trigger rate for batch of traces.

        Parameters:
        - traces: ndarray of shape (N, 54, L)
        - threshold: float, amplitude threshold for triggering

        Returns:
        - pass_rate: fraction of triggered sets
        - triggered_mask: boolean array of shape (N,) indicating triggered sets
        """
        num_sets = traces.shape[0]
        triggered_mask = np.zeros(num_sets, dtype=bool)

        for i in range(num_sets):
            for j in range(traces.shape[1]):
                amp, _ = self.fit(traces[i, j])
                if amp > threshold:
                    triggered_mask[i] = True
                    break

        pass_rate = np.sum(triggered_mask) / num_sets
        return pass_rate, triggered_mask
