...
class OptimumFilter():
    ...
    def _update_state(self):
        ...
        if self._length % 2 == 0:
            self._noise_psd_unfolded = np.concatenate(([np.inf],
                                                       self._noise_psd[1:self._length//2]/2,  # Adjusted slicing
                                                       [self._noise_psd[self._length//2]],
                                                       self._noise_psd[self._length//2-1:0:-1]/2))
        else:
            self._noise_psd_unfolded = np.concatenate(([np.inf],
                                                       self._noise_psd[1:self._length//2+1]/2,  # Adjusted slicing
                                                       self._noise_psd[self._length//2:0:-1]/2))
    ...
    def fit(self, trace):
        ...
        chisq0 = np.real(
            np.einsum('ij,ij->i', trace_fft.conjugate() / self._noise_psd_unfolded, trace_fft)
