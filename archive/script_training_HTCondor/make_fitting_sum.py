...
from archive.script_training_HTCondor.OptimumFilter import OptimumFilter
...
# -------------------
def main() -> int:
    in_pattern = "/ceph/dwong/trigger_samples/v2_300k_80ev/traces_energy_{energy}.zst"
    ...
    n_traces = 56
    samples = 300000
    energies = list(range(0, 81, 2)) 
    out_dir = Path("/home/dwong/DELight_mtr/trigger_study/pt_max_amp")
    out_dir.mkdir(parents=True, exist_ok=True)

    for E in tqdm(energies, desc="Energies", unit="E"):
        in_path = Path(in_pattern.format(energy=E))
        out_path = out_dir / f"pt_energy_{E}.npy"  # shape (repeats, 2)
